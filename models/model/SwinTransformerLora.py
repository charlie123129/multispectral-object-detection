...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0. , rank=32,h=1):
        ...
        # 使用LoRA层替换原始的Linear层来计算q、k、v
        #self.lora_qkv = LoRALayer(input_dim=dim, output_dim=dim * 3, rank=rank, h=num_heads)

        self.q_proj = LoRALayer(dim, dim, rank, h)
        self.k_proj = LoRALayer(dim, dim, rank, h)
        self.v_proj = LoRALayer(dim, dim, rank, h)
    ...
    def forward(self, x, mask=None):
        ...
        #q, k, v = qkv[0], qkv[1], qkv[2]  # 拆分LoRA层的输出为q、k、v
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,rank=32,h=1):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        #self.fc1 = nn.Linear(in_features, hidden_features)
        self.fc1 = LoRALayer(in_features, hidden_features, rank, h)

        self.act = act_layer()
        #self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = LoRALayer(hidden_features, out_features, rank, h)
