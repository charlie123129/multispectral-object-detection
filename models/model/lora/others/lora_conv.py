...
class ConvLoRALayer(nn.Module):
    def __init__(self, in_channels, kernel_size, rank, alpha=1.0, stride=1, padding=0, dilation=1):
        super(ConvLoRALayer, self).__init__()
        # 注意这里out_channels设置为in_channels，以保持通道数不变
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size, stride, padding, dilation)
        self.lora = LoRALayer(in_channels, in_channels, rank, alpha)  # LoRA层也相应地调整
        self.rank = rank
        self.alpha = alpha
...
class GPT_LORA_CONV(nn.Module):
    def __init__(self, in_channels, d_model, h=8, block_exp=4, n_layer=8, vert_anchors=8, horz_anchors=8,
        ...
        super(GPT_LORA_CONV, self).__init__()
        ...
        self.apply(GPT_LORA_CONV._init_weights)
