...
class SelfAttention(nn.Module):
    """
     single-head masked self-attention layer
    """
    ...
    def __init__(self, d_model, d_k, d_v, h, attn_pdrop=0.2, resid_pdrop=0.2):
        ...
        self.que_proj = nn.Linear(d_model, d_model)
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        b_s, seq_len, _ = x.shape

        # Linear projections
        q = self.que_proj(x)  # (b_s, seq_len, d_model)
        k = self.key_proj(x)  # (b_s, seq_len, d_model)
        v = self.val_proj(x)  # (b_s, seq_len, d_model)

        # Self-Attention
        #  :math:`(\text(Attention(Q,K,V) = Softmax((Q*K^T)/\sqrt(d_k))`
        att = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_model)  # (b_s, seq_len, seq_len)
        ...
        # output
        out = torch.matmul(att, v)  # (b_s, seq_len, d_model)
