...
class SelfAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, attn_pdrop=0.2, resid_pdrop=0.2, lora_rank=32, lora_alpha=1.0):
        ...
        self.que_proj = LoRALayer(d_model, h * self.d_k, self.lora_rank, self.lora_alpha)
        self.key_proj = LoRALayer(d_model, h * self.d_k, self.lora_rank, self.lora_alpha)
        self.val_proj = LoRALayer(d_model, h * self.d_v, self.lora_rank, self.lora_alpha)
        self.out_proj = LoRALayer(h * self.d_v, d_model, self.lora_rank, self.lora_alpha)
...
class myTransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, lora_rank=32, lora_alpha=1.0):
        """
        :param d_model: Output dimensionality of the model
        :param d_k: Dimensionality of queries and keys
        :param d_v: Dimensionality of values
        ...
        # Use LoRA in SelfAttention
        self.sa = SelfAttention(d_model, d_k, d_v, h, attn_pdrop, resid_pdrop, lora_rank, lora_alpha)
...
class GPT_ATTENTION(nn.Module):
    ...
    def __init__(self, d_model, h=8, block_exp=4, n_layer=8, vert_anchors=8, horz_anchors=8,
                 embd_pdrop=0.2, attn_pdrop=0.2, resid_pdrop=0.2, lora_rank=32, lora_alpha=1.0):
        super().__init__()
        ...
        # Update transformer blocks initialization
        self.trans_blocks = nn.Sequential(*[myTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, lora_rank, lora_alpha)
