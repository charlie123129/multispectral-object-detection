...
class SelfAttention(nn.Module):
    ...
    """
    def __init__(self, d_model, d_k, d_v, h, attn_pdrop, resid_pdrop, shared_que_proj, shared_key_proj, shared_val_proj, shared_out_proj):
        ...
        self.que_proj = shared_que_proj
        self.key_proj = shared_key_proj
        self.val_proj = shared_val_proj
        self.out_proj = shared_out_proj
...
class GPT_LORA(nn.Module):
    ...
    def __init__(self, d_model, h=8, block_exp=4, n_layer=8, vert_anchors=8, horz_anchors=8,
        ...
        # 创建共享的LoRALayer实例
        shared_que_proj = LoRALayer(d_model, h * (d_model // h), lora_rank, lora_alpha)
        shared_key_proj = LoRALayer(d_model, h * (d_model // h), lora_rank, lora_alpha)
        shared_val_proj = LoRALayer(d_model, h * (d_model // h), lora_rank, lora_alpha)
        shared_out_proj = LoRALayer(h * (d_model // h), d_model, lora_rank, lora_alpha)
        ...
        # 使用共享的LoRALayer实例初始化trans_blocks
        self.trans_blocks = nn.ModuleList([
            myTransformerBlock(d_model, d_model // h, d_model // h, h, block_exp, attn_pdrop, resid_pdrop,
                               shared_que_proj, shared_key_proj, shared_val_proj, shared_out_proj)
            for _ in range(n_layer)])
