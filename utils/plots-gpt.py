...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    mosaic = np.full((int(ns * h), int(ns * w), 6), 255, dtype=np.uint8)  # init

    for i, img in enumerate(images):
        # Modify here to handle 6 channels' images, reducing them to 3 channels by averaging
        if img.shape[2] == 6:
            img_1 = img[:, :, :3]
            img_2 = img[:, :, 3:]
            img = (img_1 + img_2) / 2
            img = img.astype(np.uint8)
            images[i] = img  # update the image in the list
